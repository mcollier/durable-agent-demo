...
### 2026-03-09 — Product Identity Alignment: SKU Codes as Single Source of Truth

When multiple systems represent products (e.g., inventory by SKU, flavors by ID), alignment is critical to avoid bugs and reduce cognitive load. The project's `InventoryRepository` uses 3-letter SKU prefixes (`VNE`, `MNC`, `BBC`, etc.) as keys; the `FlavorRepository` used sequential numeric IDs (`flv-001..010`). This created a redundant mapping layer: to order flavor `flv-005` (Vanilla Exception) and check inventory, you'd need to know it maps to SKU `VNE`. Decision: **Adopt the SKU code as the canonical Flavor ID** (i.e., `FlavorId = "VNE"` instead of `"flv-005"`). This simplifies APIs, eliminates mapping ambiguity, and aligns with domain-driven design principles where a product has one identity. Impact: 9 files modified (1 core model update, 1 repository, 7 tests); breaking change for GET `/api/flavors` response and POST `/api/orders` request contracts. Mitigation: Full test suite validates the change; no database migration needed (in-memory repositories). See decision inbox and implementation plan for details.

### 2026-08-08T21:54:30.921+00:00 — Feedback blob storage requirements revision
- Updated `docs/requirements/feedback-blob-storage.md` from the rubber-duck review so UTC partitioning, retry policy, persist-before-human-review ordering, idempotency, `FeedbackId` validation, local-dev storage guidance, and serializer/test requirements were explicit before implementation.
